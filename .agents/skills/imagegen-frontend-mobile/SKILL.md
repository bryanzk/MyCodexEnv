...
description: "Use when the user asks to generate mobile app screen images, app flow concepts, or iOS/Android visual mockups. This skill is image-only; do not use for code generation, web landing pages, desktop dashboards, or brand identity boards."
---
...
Use for mobile app screen images and flow images only. Do not write SwiftUI, React Native, Flutter, HTML, or implementation plans unless the user separately asks for code after the image work. For website section images use `imagegen-frontend-web`; for implementing a website from images use `image-to-code`; for brand boards use `brandkit`.
