...
function validateSkillFrontmatter(dir, frontmatter, hasFrontmatter) {
    const errors = [];
    ...
    if (!name) {
        ...
        if (name.length > 64)
            errors.push("frontmatter.name must be at most 64 characters");
        if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(name)) {
            errors.push("frontmatter.name must use lowercase letters, numbers, and single hyphens only");
        }
        if (name !== base)
            errors.push(`frontmatter.name must match parent directory name (${base})`);
    }
    if (!description) {
...
function readReferences(skillDir, maxFileBytes) {
...
function readScripts(skillDir, maxFileBytes, includeScriptBodies) {
    ...
    return readdirSync(scriptsDir, { withFileTypes: true })
        ...
        if (includeScriptBodies)
            return readAttachedFile(skillDir, relPath, maxFileBytes);
...
function parseParams(value, where) {
...
function parseTool(entry, where) {
...
function parseTools(value, where) {
    if (value === undefined || value === null)
        return undefined;
    if (!Array.isArray(value))
        throw new Error(`${where}.tools must be an array`);
    return value.map((entry, index) => parseTool(entry, `${where}.tools[${index}]`));
}
function parseToolChoice(value, where) {
    if (value === undefined || value === null)
        return undefined;
...
function parseToolAssertion(entry, where) {
    if (!entry || typeof entry !== "object" || Array.isArray(entry)) {
        throw new Error(`${where} must be an object`);
    }
    const r = entry;
    const type = r.type;
    if (typeof type !== "string" || !TOOL_ASSERTION_TYPES.has(type)) {
    ...
    switch (type) {
        ...
        case "tool-not-called": {
            if (!name)
                throw new Error(`${where}.name is required for ${type}`);
...
function parseToolAssertions(value, where) {
    if (value === undefined || value === null)
        return undefined;
    if (!Array.isArray(value))
        throw new Error(`${where}.tool_assertions must be an array`);
    return value.map((entry, index) => parseToolAssertion(entry, `${where}.tool_assertions[${index}]`));
}
function parseAssertionString(entry, where) {
...
function parseEval(entry, evalIndex) {
    if (!entry || typeof entry !== "object" || Array.isArray(entry)) {
    ...
    return {
        ...
        files: Array.isArray(record.files) ? record.files.filter((file) => typeof file === "string") : undefined,
...
function parseDefaults(value) {
    ...
    const r = value;
    const targetParams = r.target && typeof r.target === "object" && !Array.isArray(r.target)
        ? parseParams(r.target.params, "defaults.target")
        : undefined;
    const judgeParams = r.judge && typeof r.judge === "object" && !Array.isArray(r.judge)
        ? parseParams(r.judge.params, "defaults.judge")
        : undefined;
...
function readEvalsFile(skillDir) {
...
export function loadSkill(skillDir, opts = {}) {
    ...
    if (opts.strict)
        validateSkillFrontmatter(dir, frontmatter, hasFrontmatter);
