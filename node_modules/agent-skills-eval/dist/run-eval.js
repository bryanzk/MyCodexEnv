...
function evalSlug(evalCase, index = 0) {
...
function renderSkillSystemMessage(skill) {
    const parts = [
    ...
    if (skill.references.length > 0) {
        ...
        for (const ref of skill.references)
            parts.push(attachedFileXml("reference", ref));
        parts.push(`</references>`);
    }
    if (skill.scripts.length > 0) {
        parts.push(`<scripts>`);
        for (const script of skill.scripts)
            parts.push(attachedFileXml("script", script));
        parts.push(`</scripts>`);
    }
    parts.push(`</skill>`);
    return parts.join("\n");
}
function readEvalFiles(skill, evalCase) {
    return (evalCase.files ?? []).map((relativePath) => readAttachedFile(skill.dir, relativePath));
}
function inlineFiles(user, files) {
    if (files.length === 0)
        return user;
    return [
...
export async function runEval(args) {
    if (args.modes.length === 0)
        throw new Error("runEval requires at least one mode");
    ...
    const effectiveTools = args.eval.tools ?? args.skill.defaults?.tools;
    const effectiveToolChoice = args.eval.tool_choice ?? (effectiveTools && effectiveTools.length > 0 ? "auto" : undefined);
    const effectiveTargetParams = mergeParams(args.targetParams, args.skill.defaults?.target?.params, args.eval.params);
    const effectiveJudgeParams = mergeParams(args.judgeParams, args.skill.defaults?.judge?.params);
    for (const mode of args.modes) {
        ...
        const assertions = args.eval.assertions && args.eval.assertions.length > 0
        ...
        writeRunArtifacts(runDir, timing, grading, rawOutput, [{ path: "output.txt", content: rawOutput }], {
            system,
            user: userMessage,
            judgePrompt,
            fileCount: evalFiles.length,
