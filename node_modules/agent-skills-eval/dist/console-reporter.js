...
function modeBadge(mode, paint) {
    return mode === "with_skill" ? paint("cyan", "[with_skill]") : paint("magenta", "[without_skill]");
}
function summarizeAssertions(results, paint) {
    if (results.length === 0)
        return paint("gray", "(no assertions)");
    return results
...
export function consoleReporter(options = {}) {
    ...
    function onEvalStart(e) {
        ...
        write("");
        write(`  ${paint("bold", `${id} ${label}`)}  ${modeBadge(e.mode, paint)}${fileTag}  ${paint("gray", `[${counter}]`)}`);
        if (verbose) {
            if (e.system) {
    ...
    function onEvalEnd(e) {
        ...
        if (e.grading.assertion_results.length > 0) {
            ...
            for (const [i, r] of e.grading.assertion_results.entries()) {
                ...
                if (r.evidence)
                    write(`         ${paint("gray", "evidence:")} ${clip(r.evidence, snippetLength)}`);
            }
        }
    ...
    function onSuiteEnd(e) {
        ...
        if (rs.without_skill && rs.delta) {
            ...
            const ppColor = ppDelta > 0 ? "green" : ppDelta < 0 ? "red" : "gray";
            const sign = (n, suffix = "") => `${n >= 0 ? "+" : ""}${n.toFixed(1)}${suffix}`;
            baseOut("");
            baseOut(`  ${paint("bold", "summary")}  ${paint("cyan", `with_skill ${withPct}%`)}  vs  ${paint("magenta", `without_skill ${withoutPct}%`)}`);
            baseOut(`    ${paint("gray", "\u0394")} pass-rate ${paint(ppColor, sign(ppDelta, "pp"))}  ` +
            ...
            baseOut("");
            baseOut(`  ${paint("bold", "summary")}  ${paint("cyan", `with_skill ${withPct}%`)}  ${paint("gray", `\u00b7 ${withTime}s avg \u00b7 ${withTokens} tok avg`)}`);
        }
