...
function normalizeRubricGrading(raw, assertions) {
...
function failClosed(assertions, response) {
...
function renderRubricPrompt(args, previousBadResponse) {
...
async function callJudge(provider, prompt, params) {
...
function describeToolAssertion(a) {
    if (a.description)
...
function callsByName(toolCalls, name) {
    if (!name)
        return toolCalls;
    return toolCalls.filter((c) => c.function.name === name);
}
function gradeToolAssertion(assertion, toolCalls) {
    ...
    switch (assertion.type) {
        ...
        case "tool-arg-equals": {
            ...
            for (const c of matches) {
                if (c.parsedArguments === undefined)
                    continue;
        ...
        case "tool-arg-matches": {
            ...
            try {
                ...
                return {
                    text,
                    passed: false,
                    evidence: `invalid regex /${assertion.pattern}/${assertion.flags ?? ""}: ${err instanceof Error ? err.message : String(err)}`,
                };
            }
            for (const c of matches) {
                if (c.parsedArguments === undefined)
...
export function runToolAssertions(toolCalls, toolAssertions) {
    if (!toolAssertions || toolAssertions.length === 0)
        return [];
    const calls = toolCalls ?? [];
    return toolAssertions.map((a) => gradeToolAssertion(a, calls));
}
// ─── orchestrator ────────────────────────────────────────────────────────────
export async function gradeOutputs(args) {
