...
function collectSkill(skillDir) {
    if (!statSync(skillDir).isDirectory())
        return undefined;
    const meta = readJson(path.join(skillDir, "meta.json")) ?? {
        name: path.basename(skillDir),
        slug: path.basename(skillDir),
        relPath: skillDir,
    };
    const benchmark = readJson(path.join(skillDir, "benchmark.json"));
    ...
    for (const entry of entries) {
        ...
        for (const mode of ["with_skill", "without_skill"]) {
            ...
            if (!grading || !timing)
                continue;
            const prompts = readJson(path.join(runDir, "prompts.json"));
            const toolCalls = readJson(path.join(runDir, "tool_calls.json"));
            const output = readText(path.join(runDir, "outputs", "raw_output.txt")) ||
...
function collectReport(workspace) {
    ...
    for (const entry of readdirSync(root, { withFileTypes: true })) {
        if (!entry.isDirectory())
            continue;
        if (entry.name === "report" || entry.name === ".history" || entry.name.startsWith("."))
            continue;
...
function renderAssertionsTable(grading) {
...
function renderToolCallsPanel(calls) {
