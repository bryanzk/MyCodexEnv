...
- The former prompt model router is retired. Sync moves any stale regular runtime copy into `~/.codex/runtime-backups/.../retired/hooks/` and fails closed on symlinks or other non-regular targets; model defaults now come from `codex/config.template.toml` and `codex/agents/*.toml`.
...
- **Source:** `codex/hooks/harness_guard.py` adopts top-level payload/env phase only inside governed Git roots, then resolves transcript marker, workspace-bound TTL declaration, one unambiguous repo snapshot, and `unknown`. Out-of-scope low/medium calls are allowed while high-risk and protected/persistence screening remain blocked. `apply_patch` derives all targets from patch headers and treats a mixed protected patch as one blocked call. `tool_input.phase` and `tool_input.cwd` are not authorization inputs.
