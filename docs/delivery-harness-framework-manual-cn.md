...
| 5. 路由专家 | 选择 helper、planner、TDD、QA、review、ship、docs | workflow route |
...
| 术语 | 含义 |
| --- | --- |
| DHF | Delivery Harness Framework，把 agent 工作组织成可恢复、可验证、可审计流程的框架 |
| phase | 生命周期阶段，例如 research、planning、development、validation |
| lane | 风险路径，例如 local_dev、operator_live_demo、customer_or_production |
| evidence | 验证或决策证据，通常是本地 JSONL 事件或最终回复中的四字段记录 |
| checkpoint | 可恢复状态记录，包含 phase、changed surfaces、verification、blockers、next safe task |
| next safe task | 下一次可以直接执行的安全任务 |
| write set | worker agent 被允许修改的文件或目录集合 |
| integrator | 主 agent，负责整合 worker 结果和更新全局 state |
