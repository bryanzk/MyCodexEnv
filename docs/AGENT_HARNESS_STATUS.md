...
| Hooks | done | `SessionStart`, `UserPromptSubmit` including source-stage compaction probe, `PreToolUse`, `PostToolUse`, hook sync tests | completion hook not available; new probe is not deployed before W6a approval | keep final verification gate and defer real runtime activation to W6a |
