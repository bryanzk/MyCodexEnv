...
| Guardrails | done | guard hook blocks or asks on destructive, secret, remote, dynamic-exec, phase write violations, and unreceipted configured agent dispatch | high-risk patterns and dispatch tool names need ongoing tuning | extend policy with observed false positives/negatives |
