...
| Memory | done | `docs/harness-state.md`, `codex_subconscious.py` build/brief/reflect, repo index, `scripts/harness_recover.py` | recovery and reflection remain explicit helper invocations | run recovery at session start and weekly reflect only on routine/derived records |
...
| Compaction Probe | W2a done | Phase-0 shape evidence: top-level `session_id` and `cwd` present, usage/token absent; shared `compaction_counter.py` used by scanner | W2b incremental probe and source registration not yet landed; no runtime deployment | build W2b with exact-id preference, strict heuristic fallback, and ordinal-only usage degradation |
