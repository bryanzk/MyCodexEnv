...
| Memory | done | `docs/harness-state.md`, `codex_subconscious.py` build/brief/reflect, repo index, `scripts/harness_recover.py --boundary` freshness-aware safe/unsafe/unknown verdict | recovery and reflection remain explicit helper invocations; unknown boundary is fail-closed | run recovery at session start and weekly reflect only on routine/derived records |
