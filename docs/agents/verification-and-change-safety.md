...
- Select gates by task mode and changed layer:
  ...
  - For low-risk docs or visuals whose write set is limited to root `README.md`/`AGENTS.md` or Markdown, HTML, CSS, and image files under `docs/`, run targeted content/link checks, any relevant surface check, and write-set `git diff --check`; CI runs `python3 test_runner.py --lane docs --changed <paths>` for such change sets and the scheduled full run on `main` owns the full gate. Do not run the public-nav surface gate when the public surface is unchanged.
