# Handoff: project-lifecycle-harness
...
`project-lifecycle-harness` skill. The goal is to treat the generic skill as a
...
- `codex/skills/project-lifecycle-harness/agents/openai.yaml`
- `codex/skills/shipq-lifecycle-harness/SKILL.md`

The generic skill also exists in the current runtime install:

- `/Users/kezheng/.codex/skills/project-lifecycle-harness/SKILL.md`
- `/Users/kezheng/.codex/skills/project-lifecycle-harness/agents/openai.yaml`
...
- `project-lifecycle-harness` is generic. It must not mention ShipQ, workbook
...
sed -n '1,260p' codex/skills/project-lifecycle-harness/SKILL.md
sed -n '1,80p' codex/skills/project-lifecycle-harness/agents/openai.yaml
```
...
1. Decide whether `project-lifecycle-harness` should be mentioned in
   `README.md` under global skills.
2. Confirm bootstrap or sync scripts copy `codex/skills/project-lifecycle-harness`
   into `~/.codex/skills/project-lifecycle-harness`.
3. Add or update a lightweight test if MyCodexEnv has skill-sync validation.
4. Run validation:
   - `python3 test_runner.py`
   - `python3 ~/.codex/skills/.system/skill-creator/scripts/quick_validate.py codex/skills/project-lifecycle-harness`
   - `git diff --check`
5. If the change is good, commit any follow-up docs/tests and push the branch.

## Acceptance Criteria

- `project-lifecycle-harness` remains generic and domain-free.
...
```text
请在 MyCodexEnv 项目中继续处理通用 Codex skill：project-lifecycle-harness。
...
5. codex/skills/project-lifecycle-harness/SKILL.md
6. codex/skills/project-lifecycle-harness/agents/openai.yaml

目标：
- 审查 project-lifecycle-harness 是否适合作为通用生命周期 harness skill。
- 保持它 generic，不要加入 ShipQ 或其他项目专属路径。
- 检查 MyCodexEnv 的 bootstrap/sync/test 机制是否会把 codex/skills/project-lifecycle-harness 正确同步到 ~/.codex/skills/project-lifecycle-harness。
- 如有必要，更新 README/docs/test_runner.py 或相关同步测试。
- 运行 fresh verification，并在最终回复里给出 command、exit_code、key_output、timestamp。

注意：
- MyCodexEnv 当前已有提交 e49f3d8 Add lifecycle harness skills，包含 project-lifecycle-harness 和 shipq-lifecycle-harness。
