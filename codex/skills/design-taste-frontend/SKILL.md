...
description: "Default frontend taste skill for building polished landing pages, portfolios, editorial pages, and visually important web UI code when no narrower taste/image skill applies. Do not use for dashboards/data tables, mobile image generation, brand-kit images, Stitch DESIGN.md files, or exact v1 compatibility."
---
...
Use this as the default coding skill for visually important web pages when the user wants implementation. It wins over aesthetic preset skills unless the user explicitly asks for that named aesthetic or the brief strongly requires it. Route elsewhere when the task is narrower: `image-to-code` for image-first implementation, `imagegen-frontend-web` for web images only, `imagegen-frontend-mobile` for mobile screen images, `brandkit` for identity boards, `stitch-design-taste` for Stitch `DESIGN.md`, `redesign-existing-projects` for in-place audits, and `design-taste-frontend-v1` for exact legacy behavior.
