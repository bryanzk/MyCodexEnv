---
name: gpt-taste
description: "Use only when the user asks for an Awwwards-style, GSAP-heavy, cinematic landing page with explicit motion choreography and experimental layout variance. Do not use for normal frontend tasks, dashboards, accessibility-first sites, image-only work, or concise fixes."
---
...
This is a maximalist motion direction skill. Load it only when the user wants GSAP, scroll choreography, Awwwards-style experimentation, or a highly cinematic page. For default polished frontend implementation, use `design-taste-frontend`. For image-first implementation, use `image-to-code`. For restrained or regulated interfaces, do not use this skill.
...
LLMs tend to pick the first layout option. To prevent this, make a deterministic internal selection in your `<design_plan>` before writing any UI code. Do not claim that Python or any external tool was run unless you actually ran it.
Use a deterministic seed (e.g., character count of the user prompt modulo math) to select:
- 1 Hero Architecture (from Section 3)
- 1 Typography Stack (Satoshi, Cabinet Grotesk, Outfit, or Geist. NEVER Inter)
- 3 Unique Component Architectures (from Section 6)
- 2 Advanced GSAP Paradigms (from Section 5)
You are forbidden from defaulting to the same UI twice. You must follow the selected combination.
...
- **Hero Layout Options (Deterministically Selected):**
...
1. **Deterministic selection:** Write a 3-line design-selection summary showing the chosen Hero Layout, Component Arsenal, GSAP animations, and Fonts based on the prompt's character count. Do not present it as a real Python/tool execution unless a tool was actually run.
