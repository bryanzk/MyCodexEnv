...
Default to Simplified Chinese for PR summaries, change descriptions, checklist rationale, and user-facing explanations.
Keep branch names, commit messages, commands, file paths, template headings, issue references, and GitHub CLI syntax in English.
If the repository already has a fixed PR template language, follow the template language first.
...
5.  **Draft Description**: Create a PR description that strictly follows the
    ...
    - **Content**: Fill in the sections with clear, concise summaries of your
      changes. Use Simplified Chinese by default unless the repository template
      or project convention requires another language.
