---
name: ship
description: Use when a feature branch is believed ready to ship and the user wants an opinionated release workflow that merges main, runs tests, reviews the diff, updates release metadata, pushes, and prepares a PR.
---
...
This workflow is intentionally opinionated. If the current repository does not use `VERSION`, `CHANGELOG`, `main`, `npm run test`, or `bin/test-lane`, adapt the exact commands but preserve the sequence.

When this skill says `AskUserQuestion`, ask the user directly in chat with concise lettered options.
...
1. Read `codex/skills/review/checklist.md` in the repo checkout, or `~/.codex/skills/review/checklist.md` if the skill is synced into the Codex home. If the file cannot be read, **STOP** and report the error.
