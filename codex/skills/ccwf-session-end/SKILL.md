...
### 2. ~/.codex/workflow/memory/today.md Hot Data Layer

Append to `~/.codex/workflow/memory/today.md`:
```markdown
...
**Identify projects touched in this session** (from ~/.codex/workflow/memory/today.md, changed files, conversation content).

**goals.md update**:
- **Completed** → Remove entry (~/.codex/workflow/memory/today.md already has the record)
...
Read `~/.codex/workflow/memory/active-tasks.json`, update based on session work:

**Rules**:
- Tasks progressed → Update `context`/`next_action`/`updated`
- New multi-session tasks → Append new entry
- Completed tasks → Remove (~/.codex/workflow/memory/today.md has the record)
...
**Condition**: ~/.codex/workflow/memory/today.md has ≥3 session records for the day.

Quick scan ~/.codex/workflow/memory/today.md for 1-2 findings with sharing potential (counter-intuitive / data-driven / pitfall-to-solution).
...
~/.codex/workflow/memory/today.md updated
