---
name: office-hours
description: Use when a user wants founder-style office hours for evaluating a product idea before implementation, including startup diagnostics, builder brainstorming, and design-doc generation.
---
<!-- AUTO-GENERATED from SKILL.md.tmpl — do not edit directly -->
<!-- Regenerate: bun run gen:skill-docs -->

Adapted for Codex global skill use.

When this skill says `AskUserQuestion`, ask the user directly in chat with concise options.

## Preamble (run first)

```bash
_UPD=$(~/.codex/skills/office-hours/bin/gstack-update-check 2>/dev/null || .codex/skills/office-hours/bin/gstack-update-check 2>/dev/null || true)
[ -n "$_UPD" ] && echo "$_UPD" || true
mkdir -p ~/.gstack/sessions
touch ~/.gstack/sessions/"$PPID"
_SESSIONS=$(find ~/.gstack/sessions -mmin -120 -type f 2>/dev/null | wc -l | tr -d ' ')
find ~/.gstack/sessions -mmin +120 -type f -delete 2>/dev/null || true
_CONTRIB=$(~/.codex/skills/office-hours/bin/gstack-config get gstack_contributor 2>/dev/null || true)
_PROACTIVE=$(~/.codex/skills/office-hours/bin/gstack-config get proactive 2>/dev/null || echo "true")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
echo "PROACTIVE: $_PROACTIVE"
_LAKE_SEEN=$([ -f ~/.gstack/.completeness-intro-seen ] && echo "yes" || echo "no")
echo "LAKE_INTRO: $_LAKE_SEEN"
_TEL=$(~/.codex/skills/office-hours/bin/gstack-config get telemetry 2>/dev/null || true)
...
for _PF in ~/.gstack/analytics/.pending-*; do [ -f "$_PF" ] && ~/.codex/skills/office-hours/bin/gstack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true; break; done
```

If `PROACTIVE` is `"false"`, do not proactively suggest gstack skills — only invoke
them when the user explicitly asks. The user opted out of proactive suggestions.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `~/.codex/skills/office-hours/gstack-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined). If `JUST_UPGRADED <from> <to>`: tell user "Running gstack v{to} (just updated!)" and continue.
...
If A: run `~/.codex/skills/office-hours/bin/gstack-config set telemetry community`
...
If B→A: run `~/.codex/skills/office-hours/bin/gstack-config set telemetry anonymous`
If B→B: run `~/.codex/skills/office-hours/bin/gstack-config set telemetry off`
...
Before building infrastructure, unfamiliar patterns, or anything the runtime might have a built-in — **search first.** Read `~/.codex/skills/office-hours/ETHOS.md` for the full philosophy.
...
~/.codex/skills/office-hours/bin/gstack-telemetry-log \
...
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/codex/skills/browse/dist/browse" ] && B="$_ROOT/codex/skills/browse/dist/browse"
[ -z "$B" ] && B=~/.codex/skills/browse/dist/browse
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
```bash
source <(~/.codex/skills/office-hours/bin/gstack-slug 2>/dev/null)
```
...
```bash
source <(~/.codex/skills/office-hours/bin/gstack-slug 2>/dev/null) && mkdir -p ~/.gstack/projects/$SLUG
USER=$(whoami)
