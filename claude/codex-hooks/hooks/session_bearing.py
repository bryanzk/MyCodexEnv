...
except Exception:  # Bearing is best-effort; guard absence must not break startup.
    harness_guard = None
...
def harness_lines(payload: dict[str, Any]) -> list[str]:
    """S4 early signal: tell the model at session start whether this workspace
    is governed, what phase applies, and how to declare before writing —
    instead of letting it discover the rules at the first blocked write."""
    ...
    try:
        ...
        if not policy:
            return []
        cwd_raw = str(payload.get("cwd") or os.getcwd())
        scope = harness_guard.load_scope()
        if harness_guard.out_of_scope(payload, scope):
            return [
                f"[harness] workspace={cwd_raw}: out-of-scope — writes allowed; "
                "secret/destructive/dynamic-exec commands are still blocked."
            ]
        root = harness_guard.git_root(cwd_raw)
        phase, reason, trace = harness_guard.phase_with_trace(payload, policy, root)
        if phase == "unknown":
            return [
                f"[harness] workspace={cwd_raw}: governed, phase=unknown — repo writes "
                "will be blocked. Declare first: ~/.codex/bin/codex-task declare "
                f'implementation --reason "why" (marker_reason={reason})'
            ]
        return [
            f"[harness] workspace={cwd_raw}: governed, phase={phase} "
