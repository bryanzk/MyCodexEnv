...
def _skip_host_wrappers(lines: list[str], start: int) -> int | None:
    """Skip leading host-injected wrapper blocks like <recommended_plugins>...
    </recommended_plugins> that the host prepends to the user's first message.

    Only well-formed blocks are skipped: an opening tag alone on a line, later
    matched by its exact closing tag alone on a line. An unclosed wrapper
    fails closed (returns None -> MARKER_NOT_FOUND). The declaration stays
    positional: the marker must be the first real line after the wrappers.
    """
    index = start
    while True:
...
def _resolve_transcript(
    ...
    if root_repo is not None or current_repo is not None:
        # At least one side is inside a Git repo: require both sides to be
        # inside the same repo. A repo/non-repo split stays fail-closed.
        if root_repo is None or current_repo is None or root_repo != current_repo:
            return None, "ROOT_REPO_MISMATCH"
    else:
        # Neither side is a Git repo (plain document workspace). Eligibility
        # degrades to "same canonical directory" instead of failing outright,
        # so a declared task-mode marker can still be parsed.
...
def workspace_key(raw_cwd: Any) -> Path | None:
    """Stable workspace identity: git root inside a repo, canonical cwd outside."""
    repo = _git_root_from_cwd(raw_cwd)
    if repo is not None:
        return repo
    return _canonical_cwd(raw_cwd)
...
def resolve_self_declared(raw_cwd: Any, policy: dict[str, Any]) -> tuple[str | None, str]:
    """Fifth phase gate: a TTL-bound declaration written by `codex-task declare`.

    Every invalid, expired, or unparsable state fails closed (None)."""
    try:
        ...
        if datetime.now(timezone.utc) >= expires:
            return None, "DECLARATION_EXPIRED"
