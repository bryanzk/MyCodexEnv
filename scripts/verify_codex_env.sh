...
results+=("$(check codex_retired_router_source_absent '[[ ! -e "'"${REPO_ROOT}"'"/codex/hooks/model_router.py && ! -L "'"${REPO_ROOT}"'"/codex/hooks/model_router.py ]]')")
results+=("$(check codex_retired_router_runtime_absent '[[ ! -e "'"${CODEX_HOME}"'"/hooks/model_router.py && ! -L "'"${CODEX_HOME}"'"/hooks/model_router.py ]]')")
...
if [[ -d "${repo_skills_dir}" ]]; then
  repo_skills_count="$(find "${repo_skills_dir}" -mindepth 1 -maxdepth 1 -type d ! -name '.*' | wc -l | tr -d ' ')"
else
  repo_skills_count=0
fi
if [[ -d "${codex_skills_dir}" ]]; then
  codex_skills_count="$(find "${codex_skills_dir}" -mindepth 1 -maxdepth 1 -type d | wc -l | tr -d ' ')"
else
  codex_skills_count=0
fi
missing_managed_skills=0
if should_skip "skills_managed_present"; then
  results+=("SKIP:skills_managed_present")
elif [[ ! -d "${repo_skills_dir}" || ! -d "${codex_skills_dir}" ]]; then
