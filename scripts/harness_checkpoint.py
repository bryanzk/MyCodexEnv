...
def validate_structured_checkpoint(value: dict[str, object]) -> list[str]:
    errors: list[str] = []
    ...
    if not isinstance(constraints, list) or any(not isinstance(item, str) or not item.strip() for item in constraints):
        errors.append("--constraint values must be non-empty strings")
    ownership = value.get("ownership")
    if not isinstance(ownership, dict):
        errors.append("--ownership-json must be an object")
    elif ownership and (not isinstance(ownership.get("boundary"), str) or not ownership["boundary"].strip()):
        errors.append("--ownership-json boundary must be a non-empty string")
    next_action = value.get("next_action")
    if not isinstance(next_action, dict) or not isinstance(next_action.get("command"), str) or not next_action["command"].strip():
    ...
    if not isinstance(verification, dict) or set(verification) != {"command", "exit_code", "key_output", "timestamp", "freshness"}:
...
def append_checkpoint(args: argparse.Namespace) -> int:
    ...
    try:
        structured = structured_checkpoint(args, now_iso())
    except ValueError as exc:
        errors.append(str(exc))
    else:
        errors.extend(validate_structured_checkpoint(structured))
    if errors:
